...
function dispLoading() {
    if($("#loading").length == 0){
        $("body").append("<div id='loading'>Loading now...</div>");
    }
}
...
function post_f () {
    ...
    if (content.startsWith("setnick:")) {
        ...
        if (nick_ == "") {
            swal({
                title: "エラー - Error",
                text: "ニックネームを空白にすることは出来ません。\n使い方:setnick:[nickname]",
                icon: "error",
            });
            return;
        }
        $.cookie("nickname", nick_, { expires: 360, path: '/nullSecondSNS/',domain: 'kinoko2k.github.io', secure: true });
        swal({
            title: "成功",
            text: "変更に成功したよ！",
            icon: "success",
